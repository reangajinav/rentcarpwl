...
if(isset($_POST['ubah_foto'])){ // Jika menceklis checkbox yang ada di form ubah, lakukan :
	...
	if(move_uploaded_file($nama_gambar, $path)){ // Cek apakah gambar berhasil diupload atau tidak
		...
		$query = "SELECT tb_pelanggan.id_pelanggan, tb_pelanggan.no_ktp, tb_pelanggan.foto_pelanggan, tb_pelanggan.nama_lengkap, tb_pelanggan.tanggal_lahir, tb_pelanggan.alamat_pelanggan, tb_pelanggan.no_telepon, tb_pelanggan.status_peminjaman where id_pelanggan";
	...
	// Proses ubah data ke Database
	$query = "update tb_pelanggan set no_ktp='$no_ktp', nama_lengkap='$nama_lengkap', tanggal_lahir='$tanggal_lahir', alamat_pelanggan='$alamat_pelanggan', no_telepon='$no_telepon' , status_peminjaman='$status_peminjaman' where id_pelanggan='$id_pelanggan'";
	//var_dump($query); exit;
	$sql = mysql_query($query); // Eksekusi/ Jalankan query dari variabel $query
